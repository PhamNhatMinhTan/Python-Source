...
filename = "/home/tan/Python Source/video/test3.mp4"
file_size = (1920, 1080)  # Assumes 1920x1080 mp4
scale_ratio = 1  # Option to scale to fraction of original size.

# We want to save the output to a video file
output_filename = "/home/tan/Python Source/video/output_test3.mp4"
...
class Lane:
    ...
    def get_lane_line_indices_sliding_windows(self, plot=False):
        ...
        rightx = nonzerox[right_lane_inds]
        righty = nonzeroy[right_lane_inds]
        print("Out if: leftx: {0}, lefty: {1}, rightx: {2}, righty: {3}".format(leftx, lefty, rightx, righty))
        ...
        # Make sure we have nonzero pixels
        if len(leftx) == 0 or len(lefty) == 0 or len(rightx) == 0 or len(righty) == 0:
            leftx = np.array(prev_leftx)
            lefty = np.array(prev_lefty)
            rightx = np.array(prev_rightx)
            righty = np.array(prev_righty)
            print("In If: leftx: {0}, lefty: {1}, rightx: {2}, righty: {3}".format(leftx, lefty, rightx, righty))
